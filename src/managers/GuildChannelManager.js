...
class GuildChannelManager extends CachedManager {
   ...
   * @returns {Promise<FollowedChannelData>} Returns the followed channel data.
   */
  async addFollower(channel, targetChannel, reason) {
    const channelId = this.resolveId(channel);
    const targetChannelId = this.resolveId(targetChannel);
    if (!channelId || !targetChannelId) throw new Error('GUILD_CHANNEL_RESOLVE');
    const data = await this.client.api.channels[channelId].followers.post({
   ...
   * @property {?boolean} hasMore Whether there are potentially additional threads
   */
  ...
  async fetchActiveThreads(cache = true) {
    const data = await this.client.api.guilds(this.guild.id).threads.active.get();
    const ThreadManager = require('./ThreadManager');
    return ThreadManager._mapThreads(data, this.client, { guild: this.guild, cache });
  }
