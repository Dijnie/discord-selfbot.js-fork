...
/**
 * Represents the data of an option from a prompt of a guild's onboarding.
 * @extends {Base}
 */
class GuildOnboardingPromptOption extends Base {
  constructor(client, data, guildId) {
    ...
    this.channels = data.channel_ids.reduce(
      (channels, channelId) => channels.set(channelId, guild?.channels?.cache?.get(channelId) ?? null),
      new Collection(),
    );
    ...
    this.roles = data.role_ids.reduce(
      (roles, roleId) => roles.set(roleId, guild?.roles?.cache?.get(roleId) ?? null),
      new Collection(),
    );
...
module.exports = GuildOnboardingPromptOption;
